...
{
    public static class LookUpServiceHelper
    {
        ...
        public static ICollection<LookUpView> RegularInstitutions(IInstitutionRepository _repository)
        {
            ...
            //Load all the institution types
            var query = _repository.GetNoTrackingDbSet().Where(s => s.IsActive == true && s.Membership.IsActive == true && s.IsDeleted == false && s.Membership.MembershipAccountTypeId == Model.ModelViews.Enums.MembershipAccountCategory.Regular);  //**Parametrise Bank
            var list = _repository.FindBy(query);
            foreach (var c in list)
            {
                LookUpView lk = new LookUpView();
        ...
        public static ICollection<CollateralSubTypeView> CollateralSubTypesDetailed(ILKCollateralSubTypeCategoryRepository _repository)
        {
            ICollection<CollateralSubTypeView> lookups = new List<CollateralSubTypeView>();
            //Load all the institution types
            var query = _repository.GetDbSet().Where(s => s.IsActive == true && s.IsDeleted == false).OrderBy(s => s.CollateralSubTypeCategoryName);
            var list = _repository.FindBy(query);
            foreach (var c in list)
            {
                CollateralSubTypeView lk = new CollateralSubTypeView();
                lk.Id = (int)c.Id;
                lk.CollateralSubType = c.CollateralSubTypeCategoryName;
                lk.SerialNoName = c.SerialNumberName;
                lookups.Add(lk);
            }
            return lookups;
        }
        ...
        public static ICollection<LookUpView> LegalEntityClients(IInstitutionRepository _repository, int? MembershipId, bool loadDeactivated = false)
        {
            ...
            {
                query = _repository.GetNoTrackingDbSet().Where(s => s.IsActive == true && s.IsDeleted == false && s.Membership.IsActive == true && s.Membership.MembershipTypeId == Model.ModelViews.Enums.MembershipCategory.Client && s.MembershipId != MembershipId).OrderBy(m => m.Name);

            }
            else
            {
                query = _repository.GetNoTrackingDbSet().Where(s => s.IsActive == true && s.IsDeleted == false && s.Membership.IsActive == true && s.Membership.MembershipTypeId == Model.ModelViews.Enums.MembershipCategory.Client).OrderBy(m => m.Name);
        ...
        public static ICollection<AuditActionTypeView> AuditActionTypes(ILKAuditActionRepository _repository)
        {


            ICollection<AuditActionTypeView> lookups = new List<AuditActionTypeView>();

            //Load all the audit action types
            var query = _repository.GetNoTrackingDbSet().Where(s => s.IsDeleted == false && s.IsActive == true).OrderBy(s => s.Name);
            var list = _repository.FindBy(query);
            foreach (var c in list)
            {
                AuditActionTypeView lk = new AuditActionTypeView();
                lk.Id = (int)c.Id;
                lk.Name = c.Name;
                lk.AuditCategoryId = (int)c.AuditTypeId;
                lookups.Add(lk);
            }
