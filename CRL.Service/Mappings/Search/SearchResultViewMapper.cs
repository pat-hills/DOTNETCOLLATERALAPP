...
namespace CRL.Service.Mappings.Search
{
    ...
    {
        public static ICollection<SearchResultView> ConvertToSearchResultView(this IEnumerable<Model.FS.FinancialStatement> model,
            SearchRequest searchrequest,IParticipantRepository _participantRepository, ICollateralRepository _collateralrepository)
        {
            ...
            foreach (Model.FS.FinancialStatement fs in model)
            {
                ...
                ICollection<Participant> debtors = fs.Participants.Where(s => s.ParticipationTypeId == Model.FS.Enums.ParticipationCategory.AsBorrower).ToList();
                ...
                foreach (Participant p in debtors)
                {
                            ...
                            || !String.IsNullOrWhiteSpace(searchrequest.SearchParam.DebtorEmail )||  searchrequest.SearchParam .DebtorDateOfBirth !=null )
                        {
                            ...
                            if (!debtor .isTagged )
                            {
                                ...
                                if (!debtor.isTagged)
                                {
                                    ...
                                    List<Participant> debtorhistory = _participantRepository.GetDbSet().Where(s => s.ParticipantNo == p.ParticipantNo && s.Id != p.Id && s.FinancialStatement.isApprovedOrDenied ==1 &&  s.FinancialStatement.ClonedId == null  && s.IsDeleted ==false && s.FinancialStatement .IsDeleted ==false).ToList();
                    ...
                    else
                    {
                        ...
                        debtor.IDNo = ((InstitutionParticipant)p).CompanyNo;
                ...
                foreach (Collateral c in collaterals)
                {
                    ...
                    if (!String.IsNullOrWhiteSpace(searchrequest.SearchParam.CollateralSerialNo) || !String.IsNullOrWhiteSpace(searchrequest.SearchParam.CollateralDescription ))
                    {
                        ...
                        {
                            //Check history
                            List<Collateral> collateralhistory = _collateralrepository.GetDbSet().Where(s => s.CollateralNo  == c.CollateralNo  && s.Id != c.Id && s.FinancialStatement.isApprovedOrDenied ==1 &&  s.FinancialStatement.ClonedId == null  && s.IsDeleted ==false && s.FinancialStatement .IsDeleted ==false).ToList();
